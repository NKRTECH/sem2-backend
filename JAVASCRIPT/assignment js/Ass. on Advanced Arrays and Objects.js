...
const nw = strings.filter(function fivlet(strings){
    if (strings.length >= 5){
        return strings
    }
});console.log(nw)
...
const newnum = numbers.map(function(numbers){
    return numbers**2
});
console.log(newnum)
...
//*/***this is without using reduce method***************** */
//const alpha = alphabets.join('')     //output kalvium
//console.log(alpha)

const alpha = alphabets.reduce(concat,0)
   function concat(){
...
const rnk = ranks.findIndex(function(item){
...
const greaterthan45 = marks.some(function(i){
  return i<45
})
console.log(greaterthan45)
...
const ifeverypositive = positive.every(function(i){
  return i>0
});console.log(ifeverypositive)
